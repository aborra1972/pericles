...
Standard mood filenames are `idle.svg`, `happy.svg`, `thinking.svg`, `surprised.svg`, `funny.svg`, and `angry.svg`. Complete selectable skins also provide `speaking.svg`. A skin may provide `all-moods.svg` as a visual overview and style-specific variants such as `happy-anim.svg`.
...
During TTS playback, firmware switches the selected skin to `speaking.svg`. The speaking asset owns the looping phoneme-like mouth motion; normal mood assets do not chatter continuously. When TTS ends, firmware restores the mood that was active before speaking.
